...
'''
# source data 
sourceFile = 'Type_II_methyltransferase_genes_Protein_nonP.fasta'
sourceDir = '../DATA/bsp'

# name for saved data file. 'None' to not save output
saveFileName = 'Type_II_methyltransferase_genes_Protein_nonP_site.fasta'
saveDir = '../DATA/bsp'
...
for rec in records:
    ...
    if (len(split)==3) or (len(split)>5):
        continue
    elif (len(split)==4) and (split[-1]=='fragment'):
        continue
    else:
